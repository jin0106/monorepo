...
import ExceedMaxLengthError from '@/utils/design-system/error/ExceedMaxLengthError'
import InvalidValueError from '@/utils/design-system/error/InvalidValueError'
...
const toNumber = (
  ...
  if (!Number.isFinite(valueNewAsNumber) || valueNewAsNumber > Number.MAX_SAFE_INTEGER) {
 ...
 * @param maxLength
 */
const sliceToMaxLength = (value: string, maxLength: InputHTMLAttributes<HTMLInputElement>['maxLength']) => {
...
const joinListToString = (list: (string | undefined | null)[], joinString: string) => {
